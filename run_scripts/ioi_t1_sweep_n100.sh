...
N_TRAIN=100 # Set to a large value so all of the (200 / 400 / 100000) examples are used
...
sbatch<<EOT
#!/bin/bash -l
#SBATCH --job-name=ioi_t1_n100_${EDGE_SPARSITY}
#SBATCH --nodes=1
...
WANDB_MODE=disabled python src/prune/fpt2_ioi.py \
    ...
    --do_train \
    --do_eval \
    --dataset_path ./data/datasets/ioi-t1/ \
    ...
    --output_dir ./data/runs/ioi_t1-${TAG}-elr${ELR}-llr${LLR}-relr${RELR}-rllr${RLLR}-es${EDGE_SPARSITY}-ns${NODE_SPARSITY}-t${TOTAL}/ \
