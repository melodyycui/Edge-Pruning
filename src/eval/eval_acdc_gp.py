...
def main():
    ...
    targets = [tokenizer.encode(" " + dataset[i]['pronoun'])[0] for i in range(len(dataset))]
    distractors = [tokenizer.encode(" " + dataset[i]['corr_pronoun'])[0] for i in range(len(dataset))]
    prefix_lengths = []
    for i in range(len(dataset)):
        sentence = dataset[i]['prefix']
